...
class FaceTrackerImpl @Inject constructor(
    ...
    override fun initialize() {
        ...
        faceLandmarker = tryInitializeWithDelegate(Delegate.CPU) ?: run {
            StageLog.failure(
                StageLog.Stage.FACE_TRACKER_INIT,
                COMPONENT,
                "FaceLandmarker.createFromOptions(CPU) failed",
                lastInitError,
            )
            return
        }
    ...
    override fun inFlightStats(nowMs: Long): InFlightStats {
        ...
        return InFlightStats(
            inFlight.isBusy(nowMs),
            stats.submitted,
            stats.refused,
            stats.expired,
            inFlight.isStalled(nowMs),
        )
    }
    ...
    override fun close() {
        synchronized(closeLock) {
            ...
            runCatching { faceLandmarker?.close() }
                .onFailure { Timber.e(it, "Error closing FaceLandmarker") }
            faceLandmarker = null
    ...
    private fun handleResult(result: FaceLandmarkerResult, resultTimestampMs: Long) {
        ...
        if (faceLandmarks.isEmpty()) {
            ...
            _gazePoints.tryEmit(
                GazePoint(
                    ...
                    0.5f,
                    confidence = 0f,
                    eligibility = GazeEligibility.NOTHING,
                    timestampMs = ts,
                ),
            )
        ...
        if (landmarks.size >= CanonicalEyes.MIN_LANDMARK_COUNT && width > 0 && height > 0) {
            runCatching {
                ...
                Timber.e(it, "Gaze feature pipeline failed")
            }
        }
        ...
        val eligibility = GazeEligibilityPolicy.evaluate(
            GazeUncertainty(
                faceDetected = true,
        ...
        var jumpFactor = 1f
        if (raw != null) {
            val headCompX = if (pose?.isValid == true) {
                pose!!.yawDeg * HEAD_POSE_COMPENSATION_FACTOR
            } else {
                0f
            }
            val headCompY = if (pose?.isValid == true) {
                pose!!.pitchDeg * HEAD_POSE_COMPENSATION_FACTOR
            } else {
                0f
            }
            ...
            if (prediction != null) {
                val decision = jumpPolicy.evaluate(
                    prediction.first.coerceIn(0f, 1f),
                    prediction.second.coerceIn(0f, 1f),
                    raw.signedTravelX,
                    raw.signedTravelY,
                    ts,
                )
        ...
        val confidence = if (raw == null || eligibility.eligibility == GazeEligibility.NOTHING) {
            0f
        } else {
            eyeQuality.coerceIn(0f, 1f)
        }
        ...
        _gazeObservations.tryEmit(
            GazeObservation(
                ...
                eyeQuality,
                pose?.isValid == true,
                pose?.takeIf { it.isValid }?.confidence ?: 0f,
                raw?.binocularAgreement ?: 0f,
                featureVector,
                ts,
                true,
            ),
